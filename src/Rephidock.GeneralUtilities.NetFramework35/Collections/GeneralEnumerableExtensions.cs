...
public static class GeneralEnumerableExtensions {
	...
	/// (Fluently performs <see cref="string.Join(string, string[])"/>)
	/// </summary>
	...
	public static string JoinString<TSource>(this IEnumerable<TSource> items, char separator) {
		return JoinString(items, separator.ToString());
	}

	/// <inheritdoc cref="JoinString(char[])"/>
	public static string JoinString(this IEnumerable<char> characters) {
		char[] charactersAsArray = characters is char[] ? (char[])characters : characters.ToArray();
		return JoinString(charactersAsArray);
	}
